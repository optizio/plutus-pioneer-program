...
mkValidator dat () ctx =
    (checkSig (beneficiary1 dat) && checkBefore) ||
    (checkSig (beneficiary2 dat) && not checkBefore)
  where
    info :: TxInfo
    info = scriptContextTxInfo ctx

    checkSig :: PubKeyHash -> Bool
    checkSig k = k `elem` txInfoSignatories info

    checkBefore :: Bool
    checkBefore = to (deadline dat) `contains` txInfoValidRange info
